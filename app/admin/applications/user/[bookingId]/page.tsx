...
export default function AdminUserApplicationReviewPage() {
  ...
  const formData = booking.userApplicationFormData || {};
  const packageType = booking.packageType || 'standard';

  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-1">User Application Review</h1>
        <p className="text-sm text-gray-600">{booking.customerName} - {booking.tour?.title}</p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="font-semibold">
              {booking.userApplicationFormSubmittedAt
                ? new Date(booking.userApplicationFormSubmittedAt).toLocaleString()
                : 'N/A'}
            </p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="space-y-3">
          ...
          {packageType === 'umrah' && booking.userPassportPhoto && (
            <div className="flex items-center justify-between p-3 bg-gray-50 rounded">
          ...
          {booking.userSupportingDocuments?.length > 0 && (
            <div>
              <p className="font-semibold mb-2">Supporting Documents:</p>
              {booking.userSupportingDocuments.map((doc: any, index: number) => (
